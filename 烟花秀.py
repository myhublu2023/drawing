...
# 设置窗口大小
winScreen = py.display.Info()
screen_width = winScreen.current_w
screen_height = winScreen.current_h
...
class Firework:
    ...
    def __init__(self):
        ...
        # 生成一个表示发射出的火花的粒子对象
        self.firework = Particle(ra.randint(0, screen_width), screen_height, True, self.colour)
        # 初始化爆炸状态为 False
        self.exploded = False
        self.particles = []
        # 爆炸产生的粒子数量范围
    ...
    def show(self, win):
        # 绘制发射出的火花
        py.draw.circle(win, self.colour, (int(self.firework.pos.x), int(self.firework.pos.y)), self.firework.size)
...
class Particle:
    ...
    def show(self, win):
        ...
        py.draw.circle(win,
                       (self.colour[0],
                        self.colour[1],
                        self.colour[2], 0), (int(self.pos.x), int(self.pos.y)), self.size)
...
class Trail:
    ...
    def show(self, win):
        # 绘制痕迹
        py.draw.circle(win, self.colour, (int(self.pos.x), int(self.pos.y)), self.size)
...
def fire():
    screen = py.display.set_mode((screen_width, screen_height - 66))
    clock = py.time.Clock()
    fireworks = [Firework() for i in range(2)]
    running = True
    # 加载字体
    font = py.font.SysFont("comicsansms", 99)
    # 渲染文本
    text = "Happy New Year!"
    ...
    while running:
        ...
        text_y = (screen_height - text_height) // 2 - 99
